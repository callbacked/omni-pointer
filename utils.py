...
def annotate(image_source: np.ndarray, boxes: torch.Tensor, logits: torch.Tensor, phrases: List[str], text_scale: float, 
    ...
    """    
    This function annotates an image with bounding boxes, labels, and center points.
    """
    ...
    for box in xyxy:
        ...
        # Draw bullseye
        cv2.circle(annotated_frame, (center_x, center_y), 10, (0, 0, 255), 2)  # Outer circle
        cv2.circle(annotated_frame, (center_x, center_y), 2, (0, 0, 255), -1)  # Center dot
        
        # Add center coordinates text
        coord_text = f"({center_x}, {center_y})"
        cv2.putText(annotated_frame, coord_text, (center_x + 15, center_y),
...
def get_som_labeled_img(img_path, model=None, BOX_TRESHOLD = 0.01, output_coord_in_ratio=False, ocr_bbox=None, text_scale=0.4, text_padding=5, draw_bbox_config=None, caption_model_processor=None, ocr_text=[], use_local_semantics=True, iou_threshold=0.9,prompt=None):
    ...
    for box in filtered_boxes:
        center_x = int((box[0] + box[2])/2 * w)  # Convert back to pixel coordinates
    ...
    if use_local_semantics:
        ...
        # Add center coordinates to text box IDs
        ocr_text = [f"Text Box ID {i} (center: {centers[i][0]}, {centers[i][1]}): {txt}" 
                   for i, txt in enumerate(ocr_text)]
        
        icon_start = len(ocr_text)
        parsed_content_icon_ls = []
        for i, txt in enumerate(parsed_content_icon):
            center_idx = i + icon_start
            parsed_content_icon_ls.append(
                f"Icon Box ID {str(center_idx)} ({centers[center_idx][0]}, {centers[center_idx][1]}): {txt}")
        parsed_content_merged = ocr_text + parsed_content_icon_ls
    else:
        # Add center coordinates to text box IDs even when not using local semantics
        ocr_text = [f"Text Box ID {i} (center: {centers[i][0]}, {centers[i][1]}): {txt}" 
                   for i, txt in enumerate(ocr_text)]
